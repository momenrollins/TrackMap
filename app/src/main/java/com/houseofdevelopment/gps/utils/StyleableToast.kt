...
//        See the License for the specific IS_ENGLISH governing permissions and
...
class StyleableToast : LinearLayout {
    ...
    private var toast_gravity: Int = 0
    ...
    private constructor(builder: Builder) : super(builder.context) {
        ...
        this.toast_gravity = builder.gravity
    ...
    private fun createAndShowToast() {
        inflateToastLayout()
        toast = Toast(context)
        toast!!.setGravity(toast_gravity, 0, if (toast_gravity == Gravity.CENTER) 0 else toast!!.yOffset)
    ...
    private fun loadShapeAttributes() {
        ...
        cornerRadius =
            typedArray!!.getDimension(R.styleable.StyleableToast_stRadius, defaultCornerRadius.toFloat()).toInt()
        length = typedArray!!.getInt(R.styleable.StyleableToast_stLength, 0)
        toast_gravity = typedArray!!.getInt(R.styleable.StyleableToast_stGravity, Gravity.BOTTOM)

        if (toast_gravity == 1) {
            toast_gravity = Gravity.CENTER
        } else if (toast_gravity == 2) {
            toast_gravity = Gravity.TOP
        }
    ...
    class Builder(val context: Context) {
        ...
        fun stroke(strokeWidth: Int, @ColorInt strokeColor: Int): Builder {
            this.strokeWidth = Utils.convertPixelsToDp(strokeWidth.toFloat(), context).toInt()
        ...
        fun cornerRadius(cornerRadius: Int): Builder {
            this.cornerRadius = Utils.convertPixelsToDp(cornerRadius.toFloat(), context).toInt()
            return this
        }
